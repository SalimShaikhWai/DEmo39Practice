...
public class UserInterfaceCrudBrandService
{
    ...
    public void DisplayWithProducts(Brand b)
    {
        string header=$"{b.BrandId,-5} || {b.BrandName}";
        Console.WriteLine(header);

        foreach(var product in b.Products)
        {
            ConsoleHelper.DrawLine(160);
            ConsoleHelper.WriteText(10, product.ProductId.ToString());
            ConsoleHelper.WriteText(60,product.ProductName);
            ConsoleHelper.WriteText(10,product.CategoryId.ToString());
            ConsoleHelper.WriteText(20,product.Category?.CategoryName);
            ConsoleHelper.WriteText(10,product.ModelYear.ToString());
            ConsoleHelper.WriteText(15, product.ListPrice.ToString());
